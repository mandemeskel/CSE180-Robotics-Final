...
int main(int argc,char **argv) {
    ...
    twist_pub = nh.advertise<geometry_msgs::Twist>( TOPIC_NAME, 100 );
    ...
    while( true ) {
        ...
        if( !stop && current_location_fresh && goal_ended ) {
...
move_base_msgs::MoveBaseGoal * createGoal() {
    ...
    float theta = rng->gaussian(currentLocation.theta, 0.25);
    goal->target_pose.pose.position.x = currentLocation.x + (0.5 * cos( theta ));
    goal->target_pose.pose.position.y = currentLocation.y + (0.5 * sin( theta ));
...
void laserHandler( const sensor_msgs::LaserScan & msg ) {
    ...
    for( n = 200; n < max_angle; n++ ) {
        ...
        if( n > 360 )
            new_theta = 0.0698132; // 4degs 
            // new_theta = currentLocation.theta + 0.261799;// 0.0698132
        else
            new_theta = 0.0698132; // 4degs 
            // new_theta = currentLocation.theta - 0.261799; // 0.0698132

        // send twist message to turn away from collision
